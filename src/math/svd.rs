...
pub fn svdsecon(x: &ArrayView2<f32>, k: usize) -> (Array2<f32>, Array1<f32>, Array2<f32>) {
    let (m, n) = x.dim();
    assert!(
        k <= m && k <= n,
        "k needs to be smaller than size(X,1) and size(X,2)"
    );

    let (u, d, v) = if m <= n {
        let c = x.dot(&x.t());
        let (u, d) = c.eigen_symmetric();
        let mut v = x.t().dot(&u);
        let s = d.mapv(|x| x.abs().sqrt());
        v = v.mapv(|x| x / s);
        (u, s, v)
    } else {
        let c = x.t().dot(&x);
        let (v, d) = c.eigen_symmetric();
        let mut u = x.dot(&v);
        let s = d.mapv(|x| x.abs().sqrt());
        u = u.mapv(|x| x / s);
        (u, s, v)
